	 ...
	 $.ajax({
	    ...
	    beforeSend: function(){
	       ...
	       if(xhr && xhr.responseJSON && xhr.responseJSON.errors){
	        ...
	        var first_elem = Object.keys(errors)[0].replace('.','_'); // if detremain array values replace dot (.) to under_score
	        scrollTo(form_id+' #'+first_elem);

	         $.each(errors,function(key,value){
	         	var key = key.replace('.','_'); // if detremain array values replace dot (.) to under_score
