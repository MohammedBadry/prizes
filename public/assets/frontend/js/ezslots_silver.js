function EZSlots(id,useroptions){
	...
	this.endingLocation = 0; //location for selected symbol... needs to be a few smaller than howManySymbolsToAppend
	...
	this.init = function(){
		...
		for(var i = 0; i < this.reelCount; i++){
			...
			var jqoWindow = $('<div class="window window_"'+i+' style="background: url(../images/silver/bg.png)"></div>');
	...
	this.addSymbolsToStrip = function(jqoSlider, whichReel, shouldWin, isInitialCall){
		...
		for(var i = 0; i < that.howManySymbolsToAppend; i++){
			...
			if(!symbolsToUse[ctr]) {
				var jqoContent = $("<div class='content'><img src="+base_url+"/assets/frontend/images/silver/star.png></div>");
			} else {
	...
	EZSlots.prototype.set_digit=function(useroptions)
		{
		    var options = useroptions ? useroptions : {};
		    this.winningSet = options.winningSet;
		}
